...
public class Day01Test01 extends TestBase {
    ...
    public void test01(){
        ...
        softAssert.assertEquals(aktuelEurozoneYazisi, beklenenEurozoneYazisi, "Eurozone beklenen ile aktuel ayni degil, FAILED");
        softAssert.assertAll();

        // 10. DropDown listesinin su secenekleri oldugunu test edin {Select One, Australia (dollar),
        List<WebElement> dropdownWebElementler = select.getOptions();
        List<String> aktuelDropdownStringListesi = new ArrayList<>();
        for (WebElement each: dropdownWebElementler
             ) {
            aktuelDropdownStringListesi.add(each.getText());

        }
        List beklenenDropdownStringListesi = Arrays.asList("Select One", "Australia (dollar)",
